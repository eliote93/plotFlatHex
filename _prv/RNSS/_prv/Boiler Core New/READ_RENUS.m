function [F2F_R, nAY_R, nAX_R, nZ_R, nA_R, Prd_R, Pax_R, k_R] = READ_RENUS(fid1)
...
while  ~feof(fid)
    ...
    if strcmp(tline(4:8), 'fixed')
        Intro = textscan(tline, '%s', 100);
        cfsp  = sscanf(Intro{1}{2}, '%c');
        lfsp  = strcmp(cfsp, 'T');
    end
    
    if strcmp(tline(6:13), 'Assembly')
        break;
    end
end
...
if (lfsp)
    ...
    while  ~feof(fid)
        ...
        if Lgh > 15
            if strcmp(tline(11:15), 'K-EFF')
                Intro = textscan(tline, '%s', 100);
                k_R   = sscanf(Intro{1}{3}, '%f');
